...
func (r *queryResolver) ExportContact(ctx context.Context, id string) ([]byte, error) {
	user, err := auth.UserFromContext(ctx)
	if err != nil {
		log.Ctx(ctx).Warn().Err(err).Msg("user_contacts:list failed to get user from context")
		return nil, auth.ErrUnauthorized
	...
	writer, err := r.app.ExportContact(ctx, usecase.CmdExportContact{ContactId: id}, user)
	if err != nil {
		return nil, err
	}
